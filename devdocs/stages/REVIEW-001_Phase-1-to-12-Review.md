...
| M-2 | ~~**`ap` 前缀简化为 `=`**~~                  | 12    | ✅ 已修复     | 已实现 ±10% BETWEEN (number) 和 ±1 day BETWEEN (date)。修复于 2026-02-23。                                                                           |
...
| M-5 | ~~**History Bundle 无 `id` 字段**~~          | 10    | ✅ 已修复     | `HistoryBundle` 已添加 `id: string` 字段，`buildHistoryBundle()` 使用 `randomUUID()` 生成。修复于 2026-02-23。                                       |     |

### 3.3 低优先级

| #   | 问题                                   | Phase | 影响            | 建议                                                                                                                     |
| --- | -------------------------------------- | ----- | --------------- | ------------------------------------------------------------------------------------------------------------------------ |
| L-1 | ~~**Fastify error handler 类型断言**~~ | 11    | ✅ 已修复       | 使用 `isFastifyValidationError()` type guard 替代 `as Record<string, unknown>` 断言。类型安全且优雅。修复于 2026-02-23。 |
...
_本文档为 Phase 1-12 阶段性回顾，记录于 2026-02-23。后续开发应参考第 3 节（已知问题）和第 5 节（Phase 13 准备）。_
